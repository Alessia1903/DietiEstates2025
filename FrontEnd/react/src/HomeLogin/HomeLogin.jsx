...
const HomeLogin = () => {
  ...
  const cerca = useCallback(() => {
    ...
    // Salva i parametri di ricerca nella sessionStorage per la ricerca
  ...
  return (
    <div className="flex flex-col items-center p-8" style={{ fontFamily: "'Lexend', sans-serif" }}>
      ...
      {/* Barra di Ricerca */}
      <div className="search-bar w-full sm:w-auto mb-4">
        <form onSubmit={(e) => {e.preventDefault(); cerca();}} className="flex flex-wrap gap-4 items-center justify-center">
        ...
        <p className="text-white text-lg mt-8 mb-0 text-center">Filtri avanzati:</p>
        <div className="flex flex-row flex-wrap gap-4 items-center justify-center w-full mt-2 mb-2">
      ...
      <div className="w-[70%] max-w-4xl mx-auto">
        {<div id="risultati" className="flex flex-wrap justify-center w-full gap-4">
          {Array.isArray(risultati) && risultati.length === 0 ? (
      ...
      {/* Paginazione solo per la Ricerca */}
      {showResults && (
